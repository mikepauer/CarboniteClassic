...
Nx.BUILD			= "$Format:%h$"
if Nx.BUILD:find("Format:%h", 1, true) then Nx.BUILD = string.sub("1", 0, 7) end
...
Nx.Logo = "Interface\\AddOns\\CarboniteClassic\\Gfx\\Carbonite"
...
Nx.Broker = LibStub("LibDataBroker-1.1"):NewDataObject("Broker_Carbonite", {
						type = "data source",
						icon = "Interface\\AddOns\\CarboniteClassic\\Gfx\\MMBut",
						label = "Carbonite",
						text = "Carbonite",
...
function Nx:NXOnUpdate (elapsed)    
	...
	if Nx.WhatsNewUnread() then
		if Nx.Tick % 50 == 0 then
			if Nx.GlowOn then
				NXMiniMapBut:SetNormalTexture("Interface\\AddOns\\CarboniteClassic\\Gfx\\MMBut")
				Nx.GlowOn = false
			else
				NXMiniMapBut:SetNormalTexture("Interface\\AddOns\\CarboniteClassic\\Gfx\\MMButFilled")
				Nx.GlowOn = true
			end
		end
	end
...
Nx.GatherInfo = {
	["H"] = {	-- Herbs
		...
		{ 195,  "inv_misc_flower_03",L["Wintersbite"]},
	},
	["M"] = {	-- Mine node
